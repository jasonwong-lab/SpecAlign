function alignedSpectrum = RAFFT_NaN(spectra, reference, shift, lookahead, insertNaN)
...
%           insertNaN   -   Optional: If set to 1, will add NaN between 
%                           shifted segments instead of adjacent value
%   OUTPUT:	
...
for i=1:size(spectra,1)
    aligned=recurAlign(spectra(i,:),reference,shift,lookahead,insertNaN);
    alignedSpectrum(i,:)= aligned;
end

%Recursive segmentation----------------------------------------------------
function aligned = recurAlign(spectrum, reference, shift, lookahead,insertNaN)
...
if (lag == 0 && lookahead <= 0)
    ...
    if abs(lag) < length(spectrum)
        aligned = move(spectrum,lag,insertNaN);
    end
    mid = findMid(aligned);
...
%     aligned1 = recurAlign(firstSH,firstRH,shift, lookahead);
    aligned1 = recurAlign(firstSH,firstRH,shift, lookahead,insertNaN);
%     aligned2 = recurAlign(secSH,secRH,shift, lookahead);
    aligned2 = recurAlign(secSH,secRH,shift, lookahead,insertNaN);
...
%shift segments----------------------------------------------------------
function movedSeg = move(seg, lag, insertNaN)
