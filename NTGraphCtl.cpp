...
BEGIN_MESSAGE_MAP(CNTGraphCtrl,  CWnd)
...
//BEGIN_DISPATCH_MAP(CNTGraphCtrl,  CWnd)
...
//	DISP_FUNCTION(CNTGraphCtrl, "GetXMin", GetXMin, VT_R8, VTS_NONE)
//	DISP_FUNCTION(CNTGraphCtrl, "GetXMax", GetXMax, VT_R8, VTS_NONE)
//	DISP_FUNCTION(CNTGraphCtrl, "GetYMin", GetYMin, VT_R8, VTS_NONE)
//	DISP_FUNCTION(CNTGraphCtrl, "GetYMax", GetYMax, VT_R8, VTS_NONE)
//	DISP_FUNCTION(CNTGraphCtrl, "GetXAxisMinPos", GetXAxisMinPos, VT_I4, VTS_NONE)
//	DISP_FUNCTION(CNTGraphCtrl, "GetXAxisMaxPos", GetXAxisMaxPos, VT_I4, VTS_NONE)
...
//BEGIN_EVENT_MAP(CNTGraphCtrl, CWnd)
...
{
	DrawGraph(pDC, rcBounds);	
//	CWnd::OnDrawMetafile(pDC, rcBounds); // Generate debug assertion!?.
}
...
//{
//	DWORD dwFlags = CWnd::GetControlFlags();
//
//
...
//{
//	CWnd::OnResetState();  // Resets defaults found in DoPropExchange
//
...
void CNTGraphCtrl::CreateFont(CDC *pDC)
{
  ...
  if (m_fontY.m_hObject == NULL) 
  {
	...
	pOldFont  = &m_fontLabel;//SelectFontObject(pDC, m_fontLabel);
	...
	pOldFont  = &m_fontIdent;//SelectFontObject(pDC, m_fontIdent);
	pLogFont  = pDC->SelectObject(pOldFont);
...
void CNTGraphCtrl::CalcRect(CDC *pDC)
{
	...
	pOldFont = &m_fontTick;//SelectFontObject(pDC, m_fontTick);
	...
	pSaveFont = &m_fontTitle;//SelectFontObject(pDC,m_fontTitle);
...
void CNTGraphCtrl::DrawTitle(CDC *pDC)
{
	...
	CFont* pOldFont;
	TEXTMETRIC tm;
	const CString& strCaption =CString("");// = InternalGetText();

	pOldFont =&m_fontTitle;//SelectFontObject (pDC,m_fontTitle);
...
double CNTGraphCtrl::GetXMin()
{
	return dRangeX[MIN];
}
double CNTGraphCtrl::GetXMax()
{
	return dRangeX[MAX];
}
double CNTGraphCtrl::GetYMin()
{
	return dRangeY[MIN];
}
double CNTGraphCtrl::GetYMax()
{
	return dRangeY[MAX];
}

long CNTGraphCtrl::GetXAxisMinPos()
{
	return m_axisRect.left;
}

long CNTGraphCtrl::GetXAxisMaxPos()
{
	return m_axisRect.right;
}
