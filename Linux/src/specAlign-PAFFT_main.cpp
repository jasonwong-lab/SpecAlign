/***************************************************************************************************
File:		specAlign-PAFFT_main.cpp
Created:	2005/09/18
Author:		Jason W H Wong <jason.wong@chem.ox.ac.uk>
	
Purpose:	Implements the interface to the PAFFT method of specAlign.
...
int main(int argc, char *argv[])
{
  ...
  while ((c = getopt(argc,argv,"s:t:r:b:e:g:m:c:h")) != -1) {
    switch(c){
    case 's': //source file
      sflag = 1;
      in_filename = optarg;
 ...
 if (s!= 0){
    ...
    cout << "Aligning spectra by PAFFT...";
    fHandle.pafftAlign(segsize,begin,end,scale,mshift);
    cout << "Done"<<endl<<endl;
...
int usage() {
	printf("\nspecAlign-PAFFT 1.21 (17-Sept-05) \nby Jason W. H. Wong, Cartwright Lab, PTCL, Univeristy of Oxford, 2005\n");
	printf("\nUsage: specAlign-PAFFT <-s source> <-t target>\n");
	...
	printf("	-c   where \"scale\" is a float specifying the amount to scale \n");
        ...
        printf("       	-g   where \"segsize\" is an integer specifying the minimum\n");
        printf("             size of a segment for alignment (default: 500)\n");
        printf("       	-h   displays this message\n");
	printf("\nFriendly example: \n");
	printf("specAlign-PAFFT -s /home/raw_data/example -t /home/processed/example \n");
	printf("           	-b 200.0 -e 50000.0 -g 200 -m 50\n\n");
